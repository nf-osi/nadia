...
- **The portal Studies table is not the complete picture of existing projects.** Manually-curated projects in Synapse may exist for weeks before they are registered into the portal Studies table, and `alternateDataRepository` may be unpopulated during that period. Dedup logic that only checks `studies_table_id` will MISS these and produce duplicates. Before classifying any publication group as NEW, also do a secondary check against the portal **Files** table (`files_table_id`) for any of the group's accessions — if a file in the Files table references the accession (in its name or annotations), the accession is already provisioned somewhere and the group should be classified ADD/SKIP, not NEW. See `dedup-check-synapse-projects-not-only-portal-table` in `.nadia/skills/annotation_patterns.yaml` (Issue #296).
