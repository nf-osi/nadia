...
- **`sex` is exempt from best-effort / extrapolated filling.** Set `sex` only from a direct per-sample source (BioSample/ENA `sex` attribute, GEO GSM characteristic, supplementary-table column, or a paper statement that unambiguously applies to the specific sample). If sex cannot be directly verified for a file, **leave it blank — do NOT extrapolate** it from strain/cell-line default sex, model-system convention, or a cohort-level statement that does not assign sex per sample. A blank `sex` is correct when the source is silent; an inferred value is a fabrication. (Issue #272)
- **`libraryPrep`** (RNA library selection strategy) — when the Methods describe poly-A / mRNA / oligo-dT enrichment, set `libraryPrep = 'polyAselection'` (exact enum; not 'polyA'/'mRNA'). For ribo-depletion/total-RNA use the corresponding enum (e.g. `'rRNADepletion'`). This is a different field from `libraryPreparationMethod` (kit/protocol). Verify exact spelling via `fetch_schema_properties()`. (Issue #272)
