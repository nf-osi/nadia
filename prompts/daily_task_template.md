...
- **OpenNeuro** (neuroimaging — MRI/EEG; relevant for NF/NF2 vestibular-schwannoma and brain imaging). No simple keyword API: POST the GraphQL endpoint `https://openneuro.org/crn/graphql` to page through public datasets (`query{datasets(first:100,after:$cursor){edges{node{id,latestSnapshot{description{Name}}}},pageInfo{endCursor,hasNextPage}}}`) and keep those whose `Name` matches `search_terms` (fetch the README from the snapshot for borderline matches). Enumerate files with the existing `get_file_list_openneuro` (`prompts/repo_apis.md`). Low volume — expect 0–1 hits — but cheap. Prefix `openneuro:dsXXXXXX`.
- **NGDC / CNCB (China)** — GSA, GSA-Human, OMIX, NGDC BioProject. A large multi-omics archive where active Chinese NF/SWN groups deposit; its accessions (`CRA*`/`HRA*`/`PRJCA*`/`OMIX*`) never appear in GEO/SRA/ENA and NGDC mints no DataCite DOI, so they are otherwise invisible. **NGDC has no public JSON search API** (the web search is JS-rendered), so the workable path is **publication-linked, not repository-direct**: when processing NF papers (primary path), scan the PMC full text / data-availability statement with the NGDC regexes (`CRA######`, `HRA######`, `PRJCA######`, `OMIX######` — see CLAUDE.md `DATA_REPO_PATTERNS`) and resolve any hit. Once an accession is in hand, enumeration **works** via `get_file_list_ngdc` (`prompts/repo_apis.md`): NGDC has no search API, but record pages (`/gsa/browse/{CRA}`, `/omix/release/{OMIX}`) are static HTML exposing the sharded download base and per-run files (`{base}/{CRR}/{file}`) — set `contentSize` via HEAD (Standard 21). `HRA*` is GSA-Human controlled-access (no public files → `accessType=Controlled Access`, landing page, Standard 13). There is no programmatic keyword sweep — NGDC is reached **only** through accessions found in publications, so the full-text regex scan above is essential.
...
3. For each dataset in the group:
   ...
   f. On the **Dataset entity**, set `accessType`, `license`, and `dataUseModifiers` via `derive_license_and_data_use(repo, accession, source_metadata)` (`prompts/synapse_workflow.md` → "License and Data Use Derivation"); validate each against the `PortalDataset` enums; record any returned `flags` for the curation comment (Standard 23).
   g. Set provenance on each File/Link entity
4. Apply project-level annotations (study, resourceType, resourceStatus=pendingReview, pmid, doi)
5. Create wiki page using CLAUDE.md template — include the full datasets table, and pass the `derive_license_and_data_use()` result as `license_info` to `build_data_access_section()` so the wiki surfaces access type, license, and data use modifiers (Standard 23)
...
**Dataset (blocking):** entity is a direct child of the project root; `columnIds` begins with a system `id` (ENTITYID) column then a system `name` (STRING) column, then annotation columns; `items` is non-empty; a stable version exists whose label is ≥ the highest pre-existing label and whose item file versions are current; dataset name is descriptive (not a bare `{Repo}_{Accession}`); **`accessType`, `license`, and `dataUseModifiers` are all set with valid `PortalDataset` enum values** (Standard 23 — missing or empty is a blocking failure; `license=UNKNOWN` is acceptable, an empty `license` is not).
